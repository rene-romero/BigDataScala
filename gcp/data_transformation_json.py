...
def keys_from_schema_txt(b, p):
    ...
    keys_2 = ast.literal_eval(keys_1)
    keys = tuple(keys_2.keys())
    return keys
...
def schema_txt(b, p):
    ...
    table_schema_2 = ast.literal_eval(table_schema_1)
    schema = str()
    for key in table_schema_2:
        schema += key + ":" + table_schema_2[key] + ","
